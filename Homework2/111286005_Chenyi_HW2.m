...
% Source_H = -sqrt(1/1)*exp(-((t-t0+dz/2/c0+0.5*dt)/tau).^2);       % H field Gaussian Pulse
h1 = figure(1);
...
for T = 1:STEPS
    ...
    Ey(30) = Source_E(T) + Ey(30); % Add a E source at position z(30)
%     Hx(30) = Source_H(T) + Hx(30); % Add a H source at position z(30)
    ...
    if mod(T,5) == 0
        ...
        plot([z(30) z(30)],[-1.5 1.5],'color','k','linewidth',3)%draw source position
        ...
        scatter(T*dt*1e9,Source_E(T),'o','markeredgecolor','k','markerfacecolor','r');hold off
        drawnow
    end
end
